...
export function JournalPage({ content, indexMode = "text", pageClassName = "", basePath = pageClassName === "collective-journal" ? "/shared-practices" : "/situated-processes", slugs = pageClassName === "collective-journal" ? ["agua-arcilla-y-registros", "arcillas-silvestres-y-materiales-del-paisaje-urbano"] : ["investigacion-situada", "laboratorio", "practica-artistica"] }: { content: Record<Language, JournalCopy>; indexMode?: "text" | "projects"; pageClassName?: string; basePath?: string; slugs?: string[] }) {
  const [language, setLanguage] = useSiteLanguage();
  const pathname = usePathname();
  const copy = content[language];
  const activeSlug = basePath && pathname.startsWith(`${basePath}/`) ? decodeURIComponent(pathname.slice(basePath.length + 1).split("/")[0]) : undefined;
  const activeIndex = activeSlug ? slugs.indexOf(activeSlug) : -1;
  const activeSection = activeIndex >= 0 ? copy.sections[activeIndex] : undefined;
  const visibleSections = activeSection ? [activeSection] : activeSlug || indexMode === "projects" ? [] : copy.sections;
  return <>
    ...
    <main className={`journal-page gallery-journal-page ${pageClassName}`}>
      {!activeSection && <header className="gallery-journal-heading"><p>{copy.archive}</p><div className="gallery-journal-heading-content"><h1>{copy.title}</h1><div className="gallery-journal-intro">{copy.intro.map((paragraph) => <p key={paragraph}><EditorialText text={paragraph} /></p>)}{copy.website && <a className="journal-external-link" href={copy.website.href} target="_blank" rel="noopener noreferrer">{copy.website.label} <ArrowIcon /></a>}</div></div>{copy.cover && <figure className="gallery-journal-cover"><ZoomableArtworkImage image={copy.cover} sizes="(max-width: 760px) 100vw, 34vw" /></figure>}</header>}
      {!activeSection && (indexMode === "projects" && basePath ? <nav className="practice-project-index" aria-label={`${copy.narratives}: ${copy.title}`}>{copy.sections.map((section, index) => <Link href={localizedHref(`${basePath}/${slugs[index]}`, language)} key={section.number}><span>{section.number}</span><div className="practice-project-card-title"><strong>{section.title}</strong><small>{copy.archive}</small></div><figure><ZoomableArtworkImage image={section.images[0]} sizes="(max-width: 760px) 100vw, 50vw" /></figure><div className="practice-project-card-meta"><small>{section.meta}</small><em>{language === "es" ? "Ver página" : "View page"} <ArrowIcon /></em></div></Link>)}</nav> : <nav className="practice-narrative-index" aria-label={`${copy.narratives}: ${copy.title}`}>{copy.sections.map((section) => <a href={`#practice-${section.number}`} key={section.number}><span>{section.number}</span><strong>{section.title}</strong></a>)}</nav>)}
      {activeSection && basePath && <Link className="practice-back-link" href={localizedHref(basePath, language)}>{language === "es" ? `Volver a ${copy.title}` : `Back to ${copy.title}`}</Link>}
      <div className="practice-narratives">{visibleSections.map((section, sectionIndex) => <section className={`practice-narrative ${section.wideText ? "is-wide-copy" : ""}`} id={`practice-${section.number}`} key={section.number}>
        <header><span>{section.number}</span><div><h2>{section.title}</h2><p>{section.meta}</p>{!activeSection && visibleSections[sectionIndex + 1] && <a className="practice-next-link" href={`#practice-${visibleSections[sectionIndex + 1].number}`}>{language === "es" ? "Ver" : "View"} {visibleSections[sectionIndex + 1].number} {visibleSections[sectionIndex + 1].title} ↓</a>}</div><div className="practice-narrative-copy">{section.text.map((paragraph) => <p key={paragraph}><EditorialText text={paragraph} /></p>)}</div></header>
