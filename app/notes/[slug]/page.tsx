...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return note ? { title: `${note.titleEs} — Brenda Ranieri`, description: note.textEs[0], alternates: { canonical: `/notes/${note.slug}` } } : {};
}
...
export default async function NotePage({ params, searchParams }: Props) {
  ...
  return <>
    ...
    <main className="note-page">
      <p className="note-kicker">{language === "es" ? "Nota de investigación" : "Research note"}</p>
      <NoteArticle note={note} language={language} />
      <aside className="piece-related-project note-related-link"><span>{language === "es" ? related.typeEs : related.typeEn}</span><Link href={localizedHref(related.href, language)}>{language === "es" ? related.labelEs : related.labelEn} <ArrowIcon /></Link></aside>
      <Link className="note-back-link" href={localizedHref("/notes", language)}>{language === "es" ? "Volver a Notas" : "Back to Notes"}</Link>
    </main>
