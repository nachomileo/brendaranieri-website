...
export default async function NotesPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  return <>
    ...
    <main className="notes-page">
      ...
      <div className="notes-index">{notes.map((note, index) => { const cover = note.images[note.coverIndex ?? 0]; return <Link href={localizedHref(`/notes/${note.slug}`, language)} key={note.slug}><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{language === "es" ? note.titleEs : note.titleEn}</h2><p>{language === "es" ? note.contextEs : note.contextEn}</p></div><figure><Image src={cover.src} alt={cover.alt} width={cover.width} height={cover.height} sizes="(max-width: 760px) 100vw, 50vw" /></figure><time dateTime={note.date}>{formatNoteDate(note.date, language)}</time></Link>; })}</div>
    </main>
