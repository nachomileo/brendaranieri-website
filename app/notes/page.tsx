...
export default async function NotesPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  return <>
    ...
    <main className="notes-page">
      ...
      <div className="notes-index">{notes.map((note, index) => <Link href={localizedHref(`/notes/${note.slug}`, language)} key={note.slug}><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{language === "es" ? note.titleEs : note.titleEn}</h2><p>{language === "es" ? note.contextEs : note.contextEn}</p></div><figure><Image src={note.images[0].src} alt={note.images[0].alt} width={note.images[0].width} height={note.images[0].height} sizes="(max-width: 760px) 100vw, 50vw" /></figure><time dateTime={note.date}>{note.year}</time></Link>)}</div>
    </main>
